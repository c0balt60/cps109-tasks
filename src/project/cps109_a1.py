...
• Arithmetic expressions (can include function calls)
• Use of if/elif/else statement
• Use of sequence types and their operations (strings, lists, tuples)
• Use of a for loop for sequence iteration (a list comprehension works here also)
• Use of a while loop for general iteration (driven by a condition)
• A user-defined function for solving some meaningful subproblem.
• Print statements for displaying output (user input is allowed, but not required)
• Read input from a file OR write output to a file.
...
Budget Tracker – Input expenses, categorize them, and show monthly summaries.

Bill Reminder CLI – Let users input recurring bills and show which ones are due soon.

Currency Converter – Fetch exchange rates using an API and convert between currencies.

Invoice Generator – Generate simple PDF invoices using reportlab or fpdf.

Time Tracker – Record how much time you spend on tasks or projects.

Tip Calculator / Split Bill Tool – A simple calculator for groups at restaurants.

Interest Calculator – Compute compound and simple interest for savings or loans.

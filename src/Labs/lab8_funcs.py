...
def max_pop(items: list[tuple[str, int]]):
    '''
...
def product_by_index(items: list[float], ids: list[int]) :
    '''
...
def check_name(first: str, last: str):
    ...
    if (first[0].isupper() and last[0].isupper()) and (first[1:].islower() and last[1:].islower()):
        return f"{last}, {first}"
    else:
        raise ValueError
...
def get_next_int(it: Iterator[int]):
